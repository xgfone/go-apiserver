...
func (le *LeaderElector) acquire(ctx context.Context) (succeeded bool) {
	...
	wait.JitterUntil(ctx, le.config.RetryPeriod, true, 1.2,
		func(c context.Context) (bool, error) {
			succeeded = le.tryAcquireOrRenew(c)
...
func (le *LeaderElector) renew(ctx context.Context) {
	period := le.config.RetryPeriod
	wait.Until(ctx, period, func(c context.Context) (bool, error) {
		timeoutCtx, timeoutCancel := context.WithTimeout(c, le.config.RenewDeadline)
		defer timeoutCancel()

		err := wait.Until(timeoutCtx, period, func(ctx context.Context) (bool, error) {
