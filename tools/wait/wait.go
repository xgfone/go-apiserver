...
//
// Notice: it is ported from the package "k8s.io/apimachinery@v0.23.5/pkg/util/wait".
package wait
...
func handlePanic(r interface{}) { log.Error("wrap a panic", "panic", r) }
func wrapPanic() {
	if r := recover(); r != nil {
		handlePanic(r)
	}
...
func Forever(f func(), period time.Duration) { Until(f, period, NeverStop) }

// Until loops until stop channel is closed, running f every period.
//
// Until is syntactic sugar on top of JitterUntil with zero jitter factor
// and with sliding = true (which means the timer for period starts after
// the f completes).
...
// factor, with sliding = false (meaning the timer for period starts at the
// same time as the function starts).
...
//
// If sliding is true, the period is computed after f runs. If it is false
// then period includes the runtime for f.
//
// Close stopCh to stop. f may not be invoked if stop channel is already closed.
// Pass NeverStop to if you don't want it stop.
	...
	for {
		...
		if !sliding {
			if t == nil {
				...
				t.Reset(jitteredPeriod)
			}
		}

		func() { defer wrapPanic(); f() }()
...
// ConditionWithContextFunc returns true if the condition is satisfied,
// or an error if the loop should be aborted.
//
...
func runConditionWithCrashProtectionWithContext(ctx context.Context,
	condition ConditionWithContextFunc) (bool, error) {
	defer wrapPanic()
...
type Backoff struct {
	...
	// amount chosen uniformly at random from the interval between zero
	// and `jitter*duration`.
	Jitter float64

	// The remaining number of iterations in which the duration parameter
	// may change (but progress can be stopped earlier by hitting the cap).
	//
	// If not positive, the duration is not changed.
	//
	// Used for exponential backoff in combination with Factor and Cap.
	Steps int

	// A limit on revised values of the duration parameter.
	//
	// If a multiplication by the factor parameter would make the duration
	// exceed the cap then the duration is set to the cap and the steps
...
// It ensures that the retry wait never exceeds the deadline specified
