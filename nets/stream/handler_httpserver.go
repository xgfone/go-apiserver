...
func (h *HTTPServerHandler) stop() { h.getHTTPServer().Shutdown(context.Background()) }
...
// to the inner handler.
func (h *HTTPServerHandler) OnServerExit(err error) { h.listener.OnServerExit(err); h.stop() }
